...
public class GaneStateUpdateYamlConverter : IYamlTypeConverter
{
    ...
    {
        { StateUpdateType.SetFlag, "set flag {0} to {1}" },
    };
        ...
        foreach (var (updateType, template) in Templates)
        {
            ...
            if (match.Success)
            {
                return new StateUpdate
                {
                    ...
                    FlagValue = match.Groups[2].Success ? match.Groups[2].Value : null,
                };
            }
        }
    ...
    public void WriteYaml(IEmitter emitter, object value, Type type, ObjectSerializer serializer)
    {
        ...
        var str = string.Format(Templates[update.Type], update.Name, update.FlagValue);
